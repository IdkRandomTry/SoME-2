...
public delegate bool ShouldAllowProgess();

public class Stage {
    public Vector2 position;
    public Vector2 scale;
    public string text;
    public ShouldAllowProgess condition;
    ...
    public Stage(Vector2 position, Vector2 scale, string text, ShouldAllowProgess condition) {
        this.position = position;
        this.scale = scale;
        this.text = text;
...
public class LevelTutorial : MonoBehaviour {
    ...
    void Update() {
        if (Input.GetMouseButtonDown(0) || Input.GetMouseButtonDown(1) || Input.GetMouseButtonDown(2)) {
            ...
            Stage s = TutorialDialogueStages[index + 1];
